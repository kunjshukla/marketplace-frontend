...
export function LoginModal({ isOpen, onClose }: LoginModalProps) {
  ...
  const handleGoogleLogin = () => {
    // For Google OAuth, we must use a real redirect, not fetch
    const backendUrl = process.env.NEXT_PUBLIC_API_URL?.replace(/\/$/, '') // Remove trailing slash
      ...
      <div className="bg-white rounded-lg max-w-md w-full p-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {isGuestLogin ? (
            <div className="text-center py-4">
              ...
              <p className="text-gray-600 mb-4">
                Welcome to our NFT Marketplace demo!
              </p>
              <p className="text-sm text-gray-500 mb-6">
                Click below to explore with demo credentials
              </p>
            </div>
          ) : (
            <>
          ...
          >
            {isLoading ? (
              ...
              `Sign In ${isGuestLogin ? 'as Demo User' : ''}`
            )}
          </button>
        </form>
        ...
        {isGuestLogin && (
          <p className="text-xs text-gray-500 text-center mt-4">
            * This is a demo environment. No real purchases will be made.
          </p>
        )}
      </div>
    </div>
